          ...
          >
            <div>
              <div className="flex flex-col items-start sm:flex-row sm:items-center justify-between">
                ...
                <p className={`px-3 py-1 text-xs text-white rounded-full ${post?.authorType === "admin" ? 'bg-blue-600' : 'bg-green-600'}`}>{post?.authorType}</p>
              </div>
              <p className="text-sm text-gray-400 mt-1">By {post.postedBy} | {post?.postedDate}</p>
              <p className="text-sm text-gray-400 mt-1 mb-4">Category : {post?.category}</p>
